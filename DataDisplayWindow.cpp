...
void DataDisplayWindow::onInspectButtonClicked()
{
	...
	QString folderName = "inspection_result"; // Name of the new folder
	...
	QFile fileOut; // Declare outside the loop to use in the loop
	QTextStream out; // Declare outside the loop to use in the loop
		...
		if (smallSeqMatch.hasMatch()) {
			...
			if (smallSequences.contains(currentSeq)) {
				...
				if (inspectSeparatelyChecked && fileOut.isOpen()) {
					fileOut.close(); // Close the individual file when no longer interested
				}
			}
		}
	...
	// If not inspecting separately, inform the user that the inspection is complete
	if (!inspectSeparatelyChecked) {
